  ...
  return (
    <div className={`dropdown ${isMobile ? "mobile" : ""}`} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div className="dropdown-label" onClick={handleClick}>
        <span className=" leading-[16px]  font-medium">{label}</span>
        ...
        {items.map((item, index) =>
          typeof item === "object" ? (
            <li key={index} className="dropdown-submenu">
              ...
              >
                <span className="submenu-text  leading-[16px]  font-medium">{item.label}</span>
                <FaAngleRight className="arrow-right" />
              </div>
              <ul className={`submenu-content ${openSubmenu === index ? "open" : ""}`}>
                {item.items.map((subItem, subIndex) => (
                  <li className="submenu-item-child  leading-[16px]  font-medium" key={subIndex}>{subItem}</li>
                ))}
              </ul>
            </li>
          ) : (
            <li key={index} className="submenu-item">
              <span className="leading-[16px]  font-medium">{item}</span>
              <FaAngleRight className="arrow-right" />
            </li>
          )
        )}
